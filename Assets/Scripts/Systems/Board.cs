...
public class Board : MonoBehaviour
{
    ...
    private void CreateDice()
    {
        ...
        for( var i = 0; i < Num*Num; i++ )
        {
            ...
            else
            {
                ...
                {
                    dice.GetComponent<SpriteRenderer>().sprite = greenDiceImages[spawnArray[i/Num][i%Num]];
                    dice.GetComponent<BoxCollider2D>().enabled = false;
    ...
    private void CheckColor()
    {
        foreach(Dice dice in diceList)
        {
            if(dice.slotsolution != dice.diceNumber)
            {
                ...
                {
                    dice.GetComponent<SpriteRenderer>().sprite = yellowDiceImages[dice.diceNumber];
                }
                else
                {
                    dice.GetComponent<SpriteRenderer>().sprite = whiteDiceImages[dice.diceNumber];
                }
            }
        } 
    }   
    ...
    private void SwapDice(Dice swap_dice)
    {
        ...
        else
        {
            ...
            if(dice.slotsolution == dice.diceNumber)
            {
                dice.GetComponent<SpriteRenderer>().sprite = greenDiceImages[dice.diceNumber];
                dice.GetComponent<BoxCollider2D>().enabled = false;
                lineSolutionList[dice.slotPos[1] ].Remove(dice.diceNumber);
                lineSolutionList[dice.slotPos[0] + Num].Remove(dice.diceNumber);
                matched ++;
            ...
            {
                swap_dice.GetComponent<SpriteRenderer>().sprite = greenDiceImages[swap_dice.diceNumber];
